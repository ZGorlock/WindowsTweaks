:: ****************************************************************************************
@echo off & title Turn on or off the camera. & mode con cols=80 lines=13 & color 17
(Net session >nul 2>&1)||(PowerShell start """%~0""" -verb RunAs & Exit /B)
:: ****************************************************************************************
Set "【Item】=Toggle Camera On or Off"
Set "【Name】=Camera_on_off"
...
echo.
:: ****************************************************************************************
Set "Folder=%ProgramData%\TenForums.com\Turn_on_or_off_the_camera"
If not exist "%Folder%" (MkDir "%Folder%")

Set "Script=%Folder%\+Turn_on_or_off_the_camera.cmd"
If exist "%Script%" (del "%Script%")
(
echo :: ****************************************************************************************
echo @echo off ^& title Turn on or off the camera. ^& mode con cols=68 lines=6 ^& color 17
echo :: ****************************************************************************************
echo ^(Net session ^>nul 2^>^&1^)^|^|^(PowerShell start """%%~0""" -verb RunAs ^& Exit /B^) 
echo cd /d "%%~dp0"
echo For /f "tokens=3" %%%%# in ^('REG QUERY "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam" /V "Value"'^) Do ^(Set ✱=%%%%#^)
echo If "%%✱%%"=="Allow" ^(goto Turn_off_the_camera^) ^& Exit
echo :: ****************************************************************************************
echo :Turn_on_the_camera
echo ^(REG ADD "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam" /V Value /T REG_SZ /D "Allow" /F^)
echo ^(Start "" "Enabled.vbs" ^& EXIT^)
echo :: ****************************************************************************************
echo :Turn_off_the_camera
echo ^(REG ADD "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam" /V Value /T REG_SZ /D "Deny" /F^)
...
echo Item = Path ^& "+Turn_on_or_off_the_camera.cmd" 
...
echo $X.visible = $true
echo $X.showballoontip^(10,"", "Camera enabled. Press the Windows + M keys for it to take effect if the camera cannot be used.",[system.windows.forms.tooltipicon]::None^)
echo $X.dispose^(^)
...
echo $X.icon = [System.Drawing.SystemIcons]::Information
echo $X.visible = $true
echo $X.showballoontip^(10,"", "Camera Disabled. Press the Windows + M keys for it to take effect if the camera is being used.",[system.windows.forms.tooltipicon]::None^)
echo $X.dispose^(^)
echo ^(New-Object Media.SoundPlayer "C:\Windows\Media\Ring06.wav"^).PlaySync^(^);
)> "%Disabled%"
:: ****************************************************************************************
